#!/usr/bin/env python3
"""RealSense 실시간: Detect(딸기) → 상단 ROI crop → CLAHE/HSV → Stem seg → 그립점.

[전체 프레임] → YOLO detect → 각 딸기 bbox 상단 ROI → 전처리 → stem seg → 그립 좌표

share 패키지용 — `weights/best.pt`(detect) + `weights/stem_best.pt`(stem) 를 같은 폴더에 둔 뒤 실행.
...
# share 폴더 내 모듈 (realsense_live.py, stem_roi_utils.py)
...
def default_det_weights() -> str:
    """detect 가중치 — share/weights 우선."""
    for p in (
        _PKG / "weights/best.pt",
        _PKG / "weights/best_832.pt",
    ):
        if p.is_file():
            return str(p)
    return str(_PKG / "weights/best.pt")


def default_stem_weights() -> str:
    """stem ROI seg 가중치 — share/weights/stem_best.pt."""
    for p in (
        _PKG / "weights/stem_best.pt",
        _PKG / "weights/stem.pt",
    ):
        if p.is_file():
            return str(p)
    return str(_PKG / "weights/stem_best.pt")
...
def parse_args() -> argparse.Namespace:
    ...
    ap.add_argument(
        ...
        type=float,
        default=0.10,
        help="ripe 박스 red_ratio 최소. 모델 ripe인데 빨갛지 않으면 unripe 표시. 0.0=비활성",
    )
...
def stem_on_fruit(
    ...
    *,
    ...
    """한 프레임: detect → ROI stem → 그립점(노란 원) 표시."""
    ih, iw = color.shape[:2]
    ann = color.copy()
    n_stem = 0
    ...
    for i in range(len(xyxy)):
        ...
        cv2.circle(ann, (px0, py0), 3, (180, 180, 180), 1)
...
def main() -> None:
    ...
    print(f"[INFO] min-red-for-ripe={args.min_red_for_ripe} (0=색체크 끔)")
