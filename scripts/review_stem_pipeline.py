#!/usr/bin/env python3
"""ROI 파이프라인 검수: 전체 stem vs Detect→ROI→Stem 비교 이미지 저장."""
...
from stem_roi_utils import preprocess_stem_roi  # noqa: E402

ROOT = Path(__file__).resolve().parents[1]
FARM_VAL = ROOT / "datasets" / "yolo_unified_seg" / "images" / "val"


def main() -> int:
    ap = argparse.ArgumentParser()
    ap.add_argument("--mode", default="val", choices=("val", "realscene"))
    ...
    img_dir = FARM_VAL if args.mode == "val" else ROOT / "datasets" / "yolo_unified_seg" / "images" / "all"
    pattern = "*" if args.mode == "val" else "realscene_*"
    ...
    paths = sorted(img_dir.glob(pattern))[: args.max]

    for ip in paths:
        bgr = cv2.imread(str(ip))
        if bgr is None:
            continue
        pipe_ann, n_roi = stem_on_fruit(bgr, det, stem, a, det.names)
        ...
        cv2.putText(combo, f"ROI pipeline stems={n_roi}", (full_ann.shape[1] + 10, 28),
